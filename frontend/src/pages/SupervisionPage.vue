      ...
      <div v-if="showAction" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showAction = null">
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">
            {{ actionType === 'record' ? '添加督办记录' : actionType === 'assign' ? '调整人员指派' : '情绪升级备案' }}
          </h3>
          ...
          <div class="flex justify-end space-x-3 mt-6 pt-4 border-t border-gray-100">
            ...
            <button @click="submitAction" class="btn-primary">{{ actionType === 'record' ? '记录督办' : actionType === 'assign' ? '确认调整' : '确认升级' }}</button>
          </div>
        </div>
      </div>
    </Teleport>
...
import { AlertTriangle, Clock, TrendingUp, ShieldAlert, RefreshCw, MessageSquare, UserPlus } from 'lucide-vue-next'
...
const actionType = ref<'record' | 'assign' | 'escalate'>('record')
const form = reactive({ type: 'overdue', action: '', remarks: '' })
...
function openAction(id: string, t: 'record' | 'assign' | 'escalate') {
  actionCaseId.value = id
  actionType.value = t
  showAction.value = 'open'
  form.type = 'overdue'; form.action = ''; form.remarks = ''
}
